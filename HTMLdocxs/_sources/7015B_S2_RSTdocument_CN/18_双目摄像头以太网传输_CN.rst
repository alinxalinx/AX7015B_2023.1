...
打开后点击lwip213，api_mode设置为RAW API，dhcp_options打开dhcp功能，lwip_memory_options选项将mem_size设置大一些，pbuf的个数设置大一些，增大缓存空间，提高效率。点击OK

.. image:: images/18_media/image24.png
...
.. image:: images/18_media/image25.png
      
进行DHCP的设置

.. image:: images/18_media/image26.png
      
至此，基本上设置完毕。

7. 之后开始调用udp的应用函数，此函数声明定义在 echo.c文件中，

.. image:: images/18_media/image27.png
...
.. image:: images/18_media/image28.png
      
回调函数的定义如下

.. image:: images/18_media/image29.png
      
以上是udp的初始化过程。

1. udp接收为udp_receive函数，前面已经设置为udp接收的回调函数，功能为接收的上位机udp命令，并判断是否是自定义的协议。如果是询问命令，启动应答。如果是控制命令，根据命令重新设置摄像头分辨率。

.. image:: images/18_media/image30.png
      
9. 在udp_receive函数中调用了transfer_data函数，作用是发送udp数据。参数pData指向将要发送数据的地址，len为发送数据的长度。

.. image:: images/18_media/image31.png
...
.. image:: images/18_media/image32.png
...
.. image:: images/18_media/image33.png
...
.. image:: images/18_media/image34.png
      
AX7015硬件连接图

.. image:: images/18_media/image35.png
      
AX7021硬件连接图（J16扩展口）

.. image:: images/18_media/image36.png
...
.. image:: images/18_media/image37.png
      
1. Run Configurations配置如下：

.. image:: images/18_media/image38.png
      
1. 串口打印信息如下，检测出网卡速度，设置的IP地址

.. image:: images/18_media/image39.png
      
6. 打开Vivado工程文件夹，打开videoshow.exe

.. image:: images/18_media/image40.png
      
软件扫描到两个摄像头，可通过勾选来选择相应的摄像头显示，点击播放

.. image:: images/18_media/image41.png
      
显示效果如下，如果想重新选择显示通路，在软件屏幕上双击，回到选择界面，再次选择要显示的图像。

.. image:: images/18_media/image42.jpeg
      
7. 打开任务管理器，可以看到网络带宽为720Mbps左右

.. image:: images/18_media/image43.png
