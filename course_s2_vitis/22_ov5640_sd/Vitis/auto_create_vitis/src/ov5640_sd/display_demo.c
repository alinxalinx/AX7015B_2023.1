...
int main(void)
{
	...
	XGpio_Initialize(&hdmi_rstn, XPAR_HDMI_RST_DEVICE_ID);   /* initialize GPIO IP */
	...
	XGpio_DiscreteWrite(&cmos_rstn, 1, 0x1);
	usleep(500000);
	i2c_reg8_write(&ps_i2c0,0x72>>1,0x08,0x35);
	i2c_reg8_write(&ps_i2c0,0x7a>>1,0x2f,0x00);
	/*
	 * Initialize Sensor
	 */
	sensor_init(&ps_i2c0);
	...
	/* Set PS LED off */
	XGpioPs_WritePin(&GpioInstance, 9, 1) ;
	...
	while(1)
	{
		if (key_flag == 2)
		{
			...
			/* Set PS LED on */
			XGpioPs_WritePin(&GpioInstance, 9, 0) ;
...
int GpioSetup(XScuGic *InstancePtr, u16 DeviceId, u16 IntrID, XGpioPs *GpioInstancePtr)
{
	...
	/* set MIO 11 as input */
	XGpioPs_SetDirectionPin(GpioInstancePtr, 11, GPIO_INPUT) ;
	/* set interrupt type */
	XGpioPs_SetIntrTypePin(GpioInstancePtr, 11, XGPIOPS_IRQ_TYPE_EDGE_RISING) ;

	/* set MIO 9 as output */
	XGpioPs_SetDirectionPin(&GpioInstance, 9, GPIO_OUTPUT) ;
	/* enable MIO 9 output */
	XGpioPs_SetOutputEnablePin(&GpioInstance, 9, GPIO_OUTPUT) ;
...
void GpioHandler(void *CallbackRef)
{
	...
	IntVal = XGpioPs_IntrGetStatusPin(GpioInstancePtr, 11) ;
	/* clear key interrupt */
